...
from odoo.addons.dcs_utils.tools.http import get_response
from odoo.addons.dcs_converter.tools import converter

_logger = logging.getLogger(__name__)

class ConverterWizard(models.TransientModel):
    
    _name = "dcs_converter.convert"
    ...
    def convert(self):
        def export(record, content, filename):
            ...
            output = record.env['dcs_converter.converter'].convert(filename, content)
            record.write({
                'state': 'download',
                'output_name': name,
            ...
            return {
                ...
                'res_model': 'dcs_converter.convert',
                'view_mode': 'form',
                'view_type': 'form',
                'res_id': record.id,
                'views': [(False, 'form')],
