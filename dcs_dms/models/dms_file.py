...
class File(models.Model):
    ...
    def max_upload_size(self):
        ...
        return int(params.get_param('dcs_dms.max_upload_size', default=25))
    ...
    def copy(self, default=None):
        ...
        if 'directory' in default:
            directory = self.env['dcs_dms.directory'].sudo().browse(default['directory'])
    ...
    def _create_reference(self, settings, path, filename, content):
        ...
        if settings.save_type == 'database':
            return self.env['dcs_dms.data_database'].sudo().create({'data': content})
        return None
