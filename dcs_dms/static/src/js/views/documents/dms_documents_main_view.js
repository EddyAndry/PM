...
odoo.define('dcs_dms_views.DocumentsMainView', function(require) {
"use strict";
...
var utils = require('dcs_web_utils.common');
var mimetype = require('dcs_web_utils.mimetype');

var Widget = require('web.Widget');
var Dialog = require('web.Dialog');

var DocumentTreeView = require('dcs_dms_views.DocumentTreeView');
var PreviewHelper = require('dcs_dms_preview_file.PreviewHelper');
...
var DocumentTreeMainView = DocumentTreeView.extend({
	cssLibs: [
        '/dcs_web_utils/static/lib/jquery-splitter/css/jquery.splitter.css',
    ],
    jsLibs: [
        '/dcs_web_utils/static/lib/jquery-splitter/js/jquery.splitter.js',
    ],
    custom_events: _.extend({}, DocumentTreeView.prototype.custom_events, {
    	tree_ready: '_treeReady',
    	tree_changed: '_treeChanged',
    }),
	template: 'dcs_dms.DocumentTreeView',
    ...
    _update_cp: function() {
    	var self = this;
    	if (!this.$buttons) {
            this.$buttons = $(QWeb.render('dcs_dms.DocumentTreeViewButtons', {
                widget: this,
            }));
    	...
    	if (!this.$pager) {
            this.$pager = $(QWeb.render('dcs_dms.DocumentTreeViewActions', {
                widget: this,
            }));
    	...
    	if (!this.$switch_buttons) {
            this.$switch_buttons = $(QWeb.render('dcs_dms.DocumentTreeViewOptions', {
                widget: this,
            }));
        }
    	if (!this.$searchview) {
            this.$searchview = $(QWeb.render('dcs_dms.DocumentTreeViewSearch', {
                widget: this,
            }));
    ...
    _preview_node: function(node) {
    	var self = this;
    	if(node.data && node.data.odoo_model === "dcs_dms.file") {
    	...
    	} else if(node.data && node.data.odoo_model === "dcs_dms.directory") {
    		self.$el.find('.mk_document_preview').html(
    				$(QWeb.render('dcs_dms.DocumentTreeViewDirectoryPreview', {
			            widget: this,
			            directory: node.data,
    				})));
    	}
    ...
    _create_selected_node: function() {
    	var node = this.controller.getSelectedItem();
    	if(node) {
    		var parent = node.data.odoo_model === "dcs_dms.file" ? this.controller.getParentNode(node) : node;
    ...
    _show_help: function() {
    	...
    	var dialog = new Dialog(this, {
            size: 'medium',
            buttons: buttons,
            $content: $(QWeb.render('dcs_dms.DocumentHelpDialogContent')),
            title: _t("Help"),
        });
    	dialog.open();
    },
...
core.action_registry.add('dcs_dms_views.documents', DocumentTreeMainView);
