...
odoo.define('dcs_dms_views.DocumentsController', function(require) {
"use strict";
...
var files = require('dcs_web_utils.files');
...
var DocumentsController = Widget.extend({
    ...
    refresh: function(message) {
    	...
    	if(message && message.ids && message.model && !message.create) {
    		var nodes = _.each(_.keys(_.chain(message.ids).map(function(id) {
    			...
    			if(message.model === 'dcs_dms.directory') {
    		...
    		if(id && message.model) {
    			var field = message.model === 'dcs_dms.directory' ? 'parent_directory' : 'directory';
    			this._rpc({
    				 fields: [field],
    				 domain: [['id', '=', id]],
    				 model: message.model,
    ...
    getSelectedDirectory: function() {
    	...
    	var directories = _.filter(selected, function(node) {
    		return node.data.odoo_model === "dcs_dms.directory";
    	});
    ...
    _checkCallback: function (operation, node, parent, position, more) {
    	if(operation === "copy_node" || operation === "move_node") {
    		...
    		if(parent.data && parent.data.odoo_model === "dcs_dms.settings") {
    			return false;
            }
    		// prevent moving a child to a file
    		if(parent.data && parent.data.odoo_model === "dcs_dms.file") {
    			return false;
            }
    	}
    ...
    _checkSelect: function(node, event) {
    	if(this.params.filesOnly && node.data.odoo_model !== "dcs_dms.file") {
    		return false;
    	}
    	return !(node.parent === '#' && node.data.odoo_model === "dcs_dms.settings");
    },
    ...
    _moveNode: function(ev) {
		...
		if(node.data.odoo_model === "dcs_dms.file") {
			vals.directory = parent.data.odoo_id;
		} else if(node.data.odoo_model === "dcs_dms.directory") {
    ...
    _copyNode: function(ev) {
		...
		if(original.data.odoo_model === "dcs_dms.file") {
			vals.directory = parent.data.odoo_id;
		} else if(original.data.odoo_model === "dcs_dms.directory") {
			vals.parent_directory = parent.data.odoo_id;
		}
		this._rpc({
