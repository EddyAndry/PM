...
class BaseModelAccessGroups(models.AbstractModel):
    ...
    _name = 'dcs_security.access_groups'
    _description = "MuK Access Groups Model"
    _inherit = 'dcs_security.access'
    ...
    def _add_magic_fields(self):
        ...
        add('groups', fields.Many2many(
            ...
            relation='dcs_groups_%s_rel' % model,
            column1='aid',
            column2='gid',
            string="Groups",
            automatic=True,
        ...
        add('complete_groups', fields.Many2many(
            _module=base,
            comodel_name='dcs_security.groups',
            relation='dcs_groups_complete_%s_rel' % model,
            column1='aid',
            column2='gid',
    ...
    @api.multi
    @api.returns('dcs_security.groups')
    def get_groups(self):
        self.ensure_one()
        groups = self.env['dcs_security.groups']
        groups |= self.groups
        return groups
    ...
    def _get_no_access_ids(self):
        ...
        if not self._strict_security:
            sql = '''
                ...
                WHERE NOT EXISTS (
                    SELECT *
                    FROM dcs_groups_complete_%s_rel r
                    WHERE r.aid = a.id
                );         
            ''' % (self._table, model)
    ...
    def _get_access_ids(self):
        ...
        sql = '''
            SELECT r.aid
            FROM dcs_groups_complete_%s_rel r
            JOIN dcs_security_groups g ON r.gid = g.id
            JOIN dcs_security_groups_users_rel u ON r.gid = u.gid
    ...
    def check_access_groups(self, operation):
        ...
        for record in self.filtered(lambda rec: rec.id not in filter_ids):
            sql = '''
                SELECT perm_%s                
                FROM dcs_groups_complete_%s_rel r
                JOIN dcs_security_groups g ON g.id = r.gid
                JOIN dcs_security_groups_users_rel u ON u.gid = g.id
