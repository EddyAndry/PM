...
odoo.define('dcs_web_utils.summernote_ext_emoji', function (require) {
'use strict';
	...
	for (var j = 0; j < chunks.length; j++) {
		...
		for (var i = 0; i < chunks[j].length; i++) {
			...
			emoList += '<div class="col-xs-3">'
					...
					+ '/dcs_web_utils/static/lib/summernote/emoji/pngs/'
					+ emo
...
$.summernote.addPlugin({
	...
	events : {
		selectEmoji : function(event, editor, layoutInfo, value) {
			...
			var img = new Image();
            img.src = '/dcs_web_utils/static/lib/summernote/emoji/pngs/' + value + '.png';
            img.alt = value;
